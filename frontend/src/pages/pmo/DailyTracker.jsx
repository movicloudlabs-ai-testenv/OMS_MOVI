...
export default function PMODailyTracker() {
  ...
  const loadDayStatus = async () => {
    ...
    try {
      setDayLoading(true);
      const res = await pmoAPI.getTrackerDayStatus({
        date: selectedDate,
  ...
  const loadPeriodEntries = async (from, to) => {
    ...
    try {
      const res = await pmoAPI.getDailyTrackerEntries({
        from, to,
        employmentType: typeFilter || undefined,
        college: collegeFilter || undefined,
      });
  ...
  const handleDownloadDate = async (e, card) => {
    ...
    try {
      ...
      const res = await pmoAPI.exportDailyTracker({ from: card.date, to: card.date, label });
  ...
  if (!canRead) {
    return <PageWrapper><AccessDenied message="You don't have permission to view the Daily Tracker." /></PageWrapper>;
  }
        ...
        {period === 'day' ? (
          ...
          <>
            <PeriodCardGrid
              ...
              exportFn={pmoAPI.exportDailyTracker}
