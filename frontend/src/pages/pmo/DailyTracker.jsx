...
export default function PMODailyTracker() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await pmoAPI.getDailyTrackerEntries(params);
  ...
  const openDetail = async (entry) => {
    ...
    try {
      const res = await pmoAPI.getUserTrackerHistory(entry.user?._id);
  ...
  if (!canRead) {
    return <PageWrapper><AccessDenied message="You don't have permission to view the Daily Tracker / EOD reports." /></PageWrapper>;
  }
