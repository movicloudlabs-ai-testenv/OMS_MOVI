...
export default function PMOMyDailyTracker() {
  ...
  const loadProjects = async () => {
    try {
      const res = await pmoAPI.getAllProjects();
  ...
  const loadForDate = async (dateStr) => {
    try {
      setLoading(true);
      const entryRes = await pmoAPI.getMyTrackerToday(dateStr === todayStr() ? undefined : dateStr);
  ...
  const loadHistory = async () => {
    try {
      const historyRes = await pmoAPI.getMyTrackerHistory();
      ...
      <div className="w-full flex flex-col gap-6 max-w-[900px] mx-auto pb-10 font-sans text-left">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-[#E2E8F0] rounded-xl shadow-sm p-6 space-y-5">
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              {saving ? 'Submitting...' : submittedForDate ? 'Update Report' : 'Submit Daily Report'}
            </button>
          </div>
        </form>
