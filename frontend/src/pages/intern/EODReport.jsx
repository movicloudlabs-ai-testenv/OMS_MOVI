...
export default function InternEODReport() {
  const [history, setHistory] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    try {
      const res = await internAPI.getMyEODHistory();
  ...
  return (
    <PageWrapper>
      <div className="w-full flex flex-col gap-6 max-w-[700px] mx-auto pb-10 font-sans text-left">
        ...
        <div className="mt-6">
          <h1 className="text-2xl font-bold text-[#0F172A]">EOD Report</h1>
          <p className="text-sm text-[#64748B] mt-1">Share a quick end-of-day update with your team.</p>
        </div>

        <EODQuickShare api={{ ...internAPI, submitEOD: async (m) => { const r = await internAPI.submitEOD(m); load(); return r; } }} />
