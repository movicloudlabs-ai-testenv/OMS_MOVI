...
import { Bug, Download, ExternalLink, Send, FileSpreadsheet, Info, Eye } from 'lucide-react';
  ...
  return (
    <PageWrapper>
      <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div><h1 className="text-2xl font-extrabold text-slate-900">Bug Sheet</h1><p className="text-sm text-slate-500">{canAdd ? 'Report bugs directly to the shared live Google Sheet.' : 'View bugs reported to the shared live Google Sheet.'}</p></div>
            </div>
          </div>
        ...
        {canAdd ? (
          <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800 flex gap-3"><Info className="w-5 h-5 shrink-0" /><span>Submitted bugs are appended to the same Google Sheet. The sheet remains the single live source, so HR/PMO can see updates immediately.</span></div>
        ) : (
