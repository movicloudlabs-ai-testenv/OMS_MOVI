...
export default function EmployeeDailyTracker() {
  ...
  const loadProjects = async () => {
    try {
      const fetcher = employeeAPI.getAllProjects || employeeAPI.getProjects;
  ...
  const loadForDate = async (dateStr) => {
    try {
      setLoading(true);
      const entryRes = await employeeAPI.getMyTrackerToday(dateStr === todayStr() ? undefined : dateStr);
  ...
  const loadHistory = async () => {
    try {
      const historyRes = await employeeAPI.getMyTrackerHistory();
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.aiCredits !== '' && form.aiCredits !== undefined && form.aiCredits !== null) {
      const val = Number(form.aiCredits);
      if (isNaN(val) || val < 0) {
        toast.error('AI Credits cannot be negative');
        return;
      }
    }
    ...
    setSaving(true);
    try {
      await employeeAPI.submitDailyTracker({
        ...form,
        date: selectedDate,
      ...
      toast.success(submittedForDate ? 'Report updated' : 'Daily report submitted!');
      setSubmittedForDate(true);
      loadHistory();
    } catch (err) {
        ...
        <form noValidate onSubmit={handleSubmit} className="bg-white border border-[#E2E8F0] rounded-xl shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" value={form.aiCredits} onChange={set('aiCredits')} placeholder="0" className="w-full border border-[#E2E8F0] rounded-md py-2 px-3 text-[13px] focus:outline-none focus:border-[#2563EB]" />
            </div>
          </div>
