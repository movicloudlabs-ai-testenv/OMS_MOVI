...
export default function EmployeeEODReport() {
  const [history, setHistory] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    try {
      const res = await employeeAPI.getMyEODHistory();
  ...
  return (
    <PageWrapper>
      <div className="w-full flex flex-col gap-6 max-w-[700px] mx-auto pb-10 font-sans text-left">
        ...
        <EODQuickShare
          allowBackdate
          api={{ ...employeeAPI, submitEOD: async (m, d) => { const r = await employeeAPI.submitEOD(m, d); load(); return r; } }}
        />
